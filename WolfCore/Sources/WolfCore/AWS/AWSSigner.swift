...
//
//struct TimeFromNow {
//    var v2Expiration: String {
//        return ""
//    }
//
//    var v4Expiration: String {
//        return ""
//    }
//}
//
...
//    }
//
//    static func make(_ type: HashType, _ data: Data) throws -> Data {
//        return Data()
//    }
//}
//
//struct URLV4Returnable {
...
//    }
//
//    static func make(_ type: HashType, _ data: Data, key: String) throws -> Data {
//        return Data()
//    }
//}
//
...
//    public init(accessKey: String, secretKey: String, region: AWSRegion, securityToken: String? = nil) {
...
//    }
//
//    public func authHeaderV4(httpMethod: HTTPMethod, urlString: String, pathPercentEncoding: CharacterSet =  CharacterSet.urlPathAllowed, queryPercentEncoding: CharacterSet = CharacterSet.urlQueryAllowed, headers: [String: String], payload: Data, mimeType: String? = nil) throws -> [String:String] {
//
//        guard let url = URL(string: urlString) else { throw Error.badURL }
//
//        let dates = getDates(date: Date())
//
//        let bodyDigest = payload |> SHA256.init |> String.init
//
//        var updatedHeaders = updateHeaders(headers: headers, url: url, longDate: dates.long, bodyDigest: bodyDigest)
//
//        if httpMethod == .put {
//            if payload.isBytes {
//                let MD5Digest = try Hash.make(.md5, payload).base64Encoded.makeString()
...
//
//        if httpMethod == .put {
//            let payloadSize = try payload.isBytes ? payload.data.count.description : payload.hashed()
...
//            updatedHeaders["x-amz-content-sha256"] = payload |> SHA256.init |> String.init
//        }
//
...
//        guard let encodedScope = credentialScope(timeStampShort: dates.short).addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) else { throw Error.unableToEncodeCredentialScope }
...
//
//        guard let hostEncodedScope = credentialScope(timeStampShort: dates.short).addingPercentEncoding(withAllowedCharacters: .urlHostAllowed) else { throw Error.unableToEncodeCredentialScope }
//
...
//
//        guard let url = URL(string: urlString) else { throw Error.badURL }
//        let stringToSign = ["GET", "", "", "\(expirationTime)", path(url: url)].joined(separator: "\n") |> UTF8.init
//
//        let stringToSignBytes = stringToSign |> Data.init
//        let signature = try (HMAC.make(.sha1, stringToSignBytes, key: secretKey) |> Base64.init |> String.init).base64Encoded.makeString().addingPercentEncoding(withAllowedCharacters: .urlHostAllowed)
//        guard let sig = signature else { throw  Error.unableToEncodeSignature }
//
...
//        guard let url = URL(string: paramURLString) else { throw Error.badURL }
...
//        guard let encodedPath = path(url: url).addingPercentEncoding(withAllowedCharacters: pathEncoding) else { throw Error.unableToEncodeURLPath }
...
//        let canonRequestHash = canonicalRequest |> Data.init |> SHA256.init |> String.init
