...
open class API {
    ...
    public func newPromise<T>(method: HTTPMethod, path: [Any]? = nil, isAuth: Bool = true, body json: JSON? = nil, successStatusCodes: [StatusCode] = [.ok], with f: @escaping (JSON) throws -> T) -> Promise<T> {
        do {
            let request = try self.newRequest(method: method, path: path, isAuth: isAuth, body: json)
            return HTTP.retrieveJSONDictionary(with: request, successStatusCodes: successStatusCodes).then { json in
                return try f(json)
    ...
    public func newPromise(method: HTTPMethod, path: [Any]? = nil, isAuth: Bool = true, body json: JSON? = nil, successStatusCodes: [StatusCode] = [.ok]) -> SuccessPromise {
        do {
            let request = try self.newRequest(method: method, path: path, isAuth: isAuth, body: json)
